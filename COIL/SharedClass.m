...
{
    NSString *start = modal.createdAt;
    //  NSString *end = modal.subscription_end_at;
    [self getDateAndTimeForNotification :modal];
    NSDateFormatter *f = [[NSDateFormatter alloc] init];
    [f setDateFormat:@"yyyy-MM-dd HH:mm:ss"];
    NSDate *startDate = [f dateFromString:start];
    NSDate *endDate = [NSDate date];
    ...
    if (day==0)
    {
        ...
        if (hour >0) {
            TimeString=[NSString stringWithFormat:@"%ld hours ago",(long)hour];
        }
        else
            if (minutes>0) {
                TimeString=[NSString stringWithFormat:@"%ld mins ago",(long)minutes];
            }
            else
                TimeString=[NSString stringWithFormat:@"%ld secs ago",(long)seconds];
    }
    else if (day==1)
    {
        TimeString=[NSString stringWithFormat:@"yesterday at: %@ ",Time];
...
-(void)getDateAndTimeForNotification:(NotificationsModal*)modal
{
    NSArray *array=[modal.createdAt componentsSeparatedByString:@" "];
    NSString *str=[array objectAtIndex:1];
    NSArray *arrayTime=[str componentsSeparatedByString:@":"];
